...
import { Plus, Trash2, Plane, Train, Bus, Bed, Utensils, Camera, Map, MoreHorizontal, DollarSign, Pencil, MapPin, Calendar as CalendarIcon, AlertCircle, MessageSquare, Check, X, ChevronUp, ChevronDown } from 'lucide-react';
...
interface ItineraryProps {
  ...
  onReorderItems: (segmentId: string, items: TripItem[]) => void;
}
...
export const Itinerary: React.FC<ItineraryProps> = ({ segments, onAddSegment, onAddItem, onUpdateItem, onDeleteItem, onDeleteSegment, onReorderItems }) => {
  ...
  const [paidInputValue, setPaidInputValue] = useState<string>('0');
  const [pendingInputValue, setPendingInputValue] = useState<string>('0');
  ...
  const handleCostChange = (val: string, field: 'total' | 'paid' | 'pending' = 'total') => {
    if (field === 'total') setCostInputValue(val);
    if (field === 'paid') setPaidInputValue(val);
    if (field === 'pending') setPendingInputValue(val);
  ...
  const handleMoveItem = (segmentId: string, itemId: string, direction: 'up' | 'down') => {
    const segment = segments.find(s => s.id === segmentId);
    if (!segment) return;

    const items = [...segment.items].sort((a, b) => a.order - b.order);
    ...
    onReorderItems(segmentId, updatedItems);
  };
  ...
  const handleSaveItem = (e: React.FormEvent) => {
    ...
    if (isPaidActivity || newItem.category !== Category.ACTIVITY) {
      if (newItem.paymentStatus === PaymentStatus.PARTIAL) {
        paidAmount = Number(paidInputValue);
        pendingAmount = Number(pendingInputValue);
        finalCost = paidAmount + pendingAmount;
      } else {
        finalCost = Number(costInputValue);
        paidAmount = newItem.paymentStatus === PaymentStatus.PAID ? finalCost : 0;
        pendingAmount = newItem.paymentStatus === PaymentStatus.PENDING ? finalCost : 0;
      }
    }
    ...
    const currentOrder = editingItemId 
      ? segment?.items.find(i => i.id === editingItemId)?.order ?? 0
  ...
  const startEditing = (segmentId: string, item: TripItem) => {
    ...
    setPaidInputValue((item.paidAmount ?? 0).toString());
    setPendingInputValue((item.pendingAmount ?? 0).toString());
          ...
          <div key={segment.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
            ...
            <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white p-5 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition duration-300">
              ...
              <div className="space-y-3">
                {segment.items.sort((a,b) => (a.order - b.order || (a.date > b.date ? 1 : -1))).map((item, idx) => (
                  <div key={item.id} className="flex flex-col p-3 bg-slate-50 rounded-xl border border-slate-100 group/item hover:bg-white hover:border-blue-200 transition-all duration-200">
                    <div className="flex items-center gap-3">
                      <div className="flex flex-col gap-0.5 opacity-0 group-hover/item:opacity-100 transition-opacity">
                        <button 
                          disabled={idx === 0}
                          onClick={() => handleMoveItem(segment.id, item.id, 'up')}
                          className="p-0.5 text-slate-400 hover:text-blue-600 disabled:opacity-20"
                        >
                          <ChevronUp size={14} />
                        </button>
                        <button 
                          disabled={idx === segment.items.length - 1}
                          onClick={() => handleMoveItem(segment.id, item.id, 'down')}
                          className="p-0.5 text-slate-400 hover:text-blue-600 disabled:opacity-20"
                        >
                          <ChevronDown size={14} />
                        </button>
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-[10px] mt-1">
                          ...
                          {item.cost > 0 && (
                            <span className={`px-2 py-0.5 rounded-full font-bold border ${
                              item.paymentStatus === PaymentStatus.PAID ? 'bg-green-100 text-green-700 border-green-200' : 
                              item.paymentStatus === PaymentStatus.PARTIAL ? 'bg-blue-100 text-blue-700 border-blue-200' :
                              'bg-orange-100 text-orange-700 border-orange-200'
                            }`}>
                              ${item.cost.toLocaleString()}
                              {item.paymentStatus === PaymentStatus.PARTIAL && ` (Pagado: $${item.paidAmount?.toLocaleString()})`}
                            </span>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="mt-4 p-5 bg-white border-2 border-blue-500 rounded-2xl shadow-2xl animate-in zoom-in-95">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="flex flex-col">
                           <label className="text-[10px] font-bold text-slate-500 uppercase mb-1">Hora (Opcional)</label>
                           <input 
                            type="time" 
                            className="p-3 rounded-xl border border-slate-200 bg-slate-50 text-sm [color-scheme:light] focus:ring-2 focus:ring-blue-500 outline-none" 
                            value={newItem.time || ''} 
                      ...
                      {(isPaidActivity || newItem.category !== Category.ACTIVITY) && (
                        <div className="grid grid-cols-2 gap-3 animate-in fade-in slide-in-from-top-2">
                          ...
                          {newItem.paymentStatus === PaymentStatus.PARTIAL ? (
                            <div className="grid grid-cols-2 gap-2">
                              <div className="flex flex-col">
                                <label className="text-[10px] font-bold text-slate-500 uppercase mb-1">Pagado USD</label>
                                <div className="relative">
                                  <input 
                                    ...
                                    onChange={e => handleCostChange(e.target.value, 'paid')} 
                                  />
                                  <DollarSign size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400" />
                                </div>
                              </div>
                              <div className="flex flex-col">
                                <label className="text-[10px] font-bold text-slate-500 uppercase mb-1">Pendiente USD</label>
                                <div className="relative">
                                  <input 
                                    type="text" 
                                    placeholder="0.00" 
                                    ...
                                    onChange={e => handleCostChange(e.target.value, 'pending')} 
                                  />
                                  <DollarSign size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400" />
                                </div>
                              </div>
